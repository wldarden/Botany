...
void Plant::tick_tree(const WorldParams& world, PerfStats* /*perf*/) {
    pin_transport(*this, genome_);              // PIN-mediated polar auxin transport (global pass)
    tick_recursive(*nodes_[0], *this, world);  // DFS: local work only (produce, consume, grow)
    phloem_resolve(*this, genome_, world);      // Münch pressure-flow sugar transport
    xylem_resolve(*this, genome_, world);       // Phase 1/2 water + cytokinin transport
    flush_removals();
}
