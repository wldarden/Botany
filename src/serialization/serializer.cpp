...
void save_tick(std::ostream& out, const Engine& engine, uint32_t plant_id) {
    ...
    plant.for_each_node([&](const Node& node) {
        ...
        write_val(out, node.local().chemical(ChemicalID::Auxin));
        write_val(out, node.local().chemical(ChemicalID::Cytokinin));
        write_val(out, node.local().chemical(ChemicalID::Sugar));
